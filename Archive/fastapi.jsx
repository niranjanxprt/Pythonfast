...
const CH = [
  {
    ...
    content: [
      ...
      { t: "code", label: "Install FastAPI", v: `# Create project & virtual environment
mkdir my-fastapi-app && cd my-fastapi-app
python -m venv venv
source venv/bin/activate

# Install FastAPI (includes uvicorn server)
...
# --reload = auto-restart on code changes` },
  ...
  },
  {
    ...
    content: [
      { t: "p", v: "HTTP methods map to CRUD operations: GET (read), POST (create), PUT (update), DELETE (remove). FastAPI uses decorators like @app.get() to map functions to URLs." },
      { t: "code", label: "main.py — Full CRUD", v: `from fastapi import FastAPI
...
def delete_item(item_id: int):
    for i, item in enumerate(items_db):
        if item["id"] == item_id:
            return {"deleted": items_db.pop(i)}
    return {"error": "Not found"}` },
      { t: "note", v: "item_id: int in the path auto-validates that the ID is an integer. Send /items/abc and FastAPI returns a 422 error automatically." },
    ],
  },
  {
    id: 3, title: "Path & Query Parameters",
    content: [
      { t: "p", v: "Path parameters are part of the URL (/users/{user_id}). Query parameters come after ? (/items?skip=0&limit=10). FastAPI figures out which is which from your function signature." },
      { t: "code", label: "Path Parameters", v: `from fastapi import FastAPI, Path
from enum import Enum

app = FastAPI()

# Basic path param — type hint validates automatically
@app.get("/users/{user_id}")
def get_user(user_id: int):
    return {"user_id": user_id}

# Path() adds extra validation
@app.get("/items/{item_id}")
def get_item(
    item_id: int = Path(gt=0, le=10000)
):
...
def get_model(model_name: ModelName):
    return {"model": model_name.value}` },
      { t: "code", label: "Query Parameters", v: `from fastapi import FastAPI, Query
...
# skip and limit are query params (not in the path)
# URL: /items?skip=0&limit=2
@app.get("/items")
...
def find_item(name: str):
    return {"searching_for": name}` },
      { t: "note", v: "Simple rule: if the param name is in the URL path, it's a path param. If not, it's a query param. You can use both together." },
    ],
  },
  {
    id: 4, title: "Request Bodies & Pydantic v2",
    content: [
      { t: "p", v: "When clients send JSON data (POST/PUT), FastAPI uses Pydantic models to define the expected shape. Pydantic v2 auto-validates, converts types, and gives clear error messages." },
      { t: "code", label: "models.py — Define data shapes", v: `from pydantic import BaseModel, Field, field_validator
from typing import Optional
...
class User(BaseModel):
    ...
    address: Address  # nested!` },
      { t: "code", label: "main.py — Use models in endpoints", v: `from fastapi import FastAPI
...
def create_item(item: Item):
    result = item.model_dump()  # Pydantic v2: convert to dict
    if item.tax:
...
def create_product(item: ItemCreate):
...
def purchase(item_id: str, quantity: int):
  ...
  },
  {
    ...
    content: [
      { t: "p", v: "Middleware runs before/after every request. CORS middleware is essential when your React frontend (localhost:5173) calls your FastAPI backend (localhost:8000)." },
...
def root():
  ...
  },
  {
    ...
    content: [
      { t: "p", v: "Auth flow: Client sends username+password to /token, gets a JWT back, then sends it as 'Authorization: Bearer <token>' on every request. Install: pip install python-jose passlib[bcrypt]" },
...
function Chat({ username }) {
  ...
  {
    id: 12, title: "Testing with pytest",
    content: [
      { t: "p", v: "TestClient lets you test endpoints without running the server. pip install pytest httpx" },
...
const quizzes = {
  ...
  2: { q: "Which method CREATEs a resource?", o: ["GET", "POST", "PUT", "DELETE"], a: 1 },
  ...
  12: { q: "Override deps in tests how?", o: ["mock.patch()", "app.dependency_overrides[dep] = fake", "@override", "pytest.fixture()"], a: 1 },
};
